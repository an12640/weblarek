...
import { ICustomer, IOrder, IOrderResponse, IProduct } from "./types";
...
const checkoutOrderView = new Order(cloneTemplate(orderTmpl), events);

const checkoutContactsView = new Contacts(cloneTemplate(contactsTmpl), events);
...
events.on("checkout:orderUpdated", () => {
    const errors = customer.validateData();

    checkoutOrderView.render({
...
events.on("checkout:contactsUpdated", () => {
    const errors = customer.validateData();

   checkoutContactsView.render({
        errors: {
            email: errors.email,
            phone: errors.phone
        },
...
events.on("cart:checkout", () => {
    ...
    modalView.content = checkoutOrderView.render({
        payment: customer.payment,
...
events.on("order:submit", () => {
    ...
    modalView.content = checkoutContactsView.render({
...
events.on("contacts:submit", () => {
    ...
    manager
        ...
        .then((response: IOrderResponse) => {
            ...
            modalView.content = success.render({ amount: response.total });
